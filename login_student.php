...
$password=$_POST['pass_s'];
       ...
       $sql = "SELECT * FROM student WHERE (email='$email' or name='$email') AND password='$password' ";
                ...
                if($num > 0)
                {
                  $files=mysqli_fetch_array($res,MYSQLI_ASSOC);
                    $s=$files['name'];
                    $_SESSION['user'] = $s;
                    echo '<script type="text/javascript">{window.location.href = "student/student_dashboard.php"; }</script>';
                }
                else
                {
